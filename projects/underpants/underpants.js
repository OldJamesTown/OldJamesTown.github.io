...
// assign a variable to the array created by running filter on our array. these are values
// that pass the given function 
//     let passingValues = _.filter(array, test);
// // create an empty array to store failing values    
//     let rejectedValues = [];
// // loop through array, we'll check if each element is also contained in passingValues.
// // if not, push the value to the rejects array
//     for (let i = 0; i < array.length; i++) {
//         if (!_.contains(passingValues, array[i])) {
//             rejectedValues.push(array[i]);
//         }
//     } return rejectedValues;
   
     //use filter to pass the array and anonymous funtion through with all its elements
    return _.filter(array,function(elements,i, a) {
...
//     results.push(_.each(collection, test));
//     return results;
}
...
//         if (_.reject(collection, test).length > 0) {
//             return false;
//         } else return true;
//     } else {
//         let result = false;
...
// test to see if a function is passed
    if (typeof test !== 'function') {
        ...
        let results = _.map(collection, test);
...
// // if seed is not given, set seed to first element in array    
//     if (!seed) {
//         seed = array[0];
//     }
// //
